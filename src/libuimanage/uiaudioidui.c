    ...
    uiwcont_t *parentwin, uiwcont_t *statusMsg)
{
  ...
  uiBoxPackStartExpandChildren (audioidint->wcont [UIAUDID_W_MAIN_VBOX], pw);
...
uiaudioidLoadData (uiaudioid_t *uiaudioid, song_t *song, dbidx_t dbidx)
{
  ...
  if (data != NULL && *data) {
    uiImageCopy (audioidint->wcont [UIAUDID_W_AUDIOID_IMG], audioidint->wcont [UIAUDID_W_MB_LOGO]);
  }
...
uiaudioidAddItem (uiaudioid_t *uiaudioid, uiwcont_t *hbox, int tagidx, int idx)
{
  ...
  uiBoxPackStartExpandChildren (hbox, rb);
